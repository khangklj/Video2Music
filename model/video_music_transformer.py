...
class VideoMusicTransformer_V1(nn.Module):
    def __init__(self, n_layers=6, num_heads=8, d_model=512, dim_feedforward=1024,
        ...
        encoder_layer = TransformerEncoderLayer_MoE(self.d_model, self.nhead, encoder_moelayer, self.dropout)
        ...
        decoder_layer = TransformerDecoderLayer_MoE(self.d_model, self.nhead, decoder_moelayer, self.dropout)
...
#         self.total_vf_dim = total_vf_dim
#         self.Linear_vis     = KANLayer(self.total_vf_dim, self.d_model)
#         self.Linear_chord     = KANLayer(self.d_model+1, self.d_model)
...
#         # Add condition (minor or major)
#         self.condition_linear = KANLayer(1, self.d_model)
...
#             KANLayer(self.d_model, self.d_model),
#             nn.Dropout(self.dropout)
#         )
#         encoder_moelayer = SharedMoELayer(expert, self.d_model, self.d_ff, self.n_experts, self.n_experts_per_token, self.dropout)
#         encoder_layer = TransformerEncoderLayer_Shared_MoE_RoPE(self.d_model, self.nhead, encoder_moelayer, self.dropout)
#         encoder = TransformerEncoder(encoder_layer, self.nlayers, encoder_norm)
...
#         decoder_layer = TransformerDecoderLayer_Shared_MoE_RoPE(self.d_model, self.nhead, decoder_moelayer, self.dropout)
#         decoder = TransformerDecoder(decoder_layer, self.nlayers, decoder_norm)
...
#         self.Wout_root       = KANLayer(self.d_model, CHORD_ROOT_SIZE)
#         self.Wout_attr       = KANLayer(self.d_model, CHORD_ATTR_SIZE)
#         self.Wout       = KANLayer(self.d_model, CHORD_SIZE)
#         self.softmax    = nn.Softmax(dim=-1)
