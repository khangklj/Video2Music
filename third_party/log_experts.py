...
counts = [
    [[0 for _ in range(6)] for _ in range(6)] for _ in range(n_layers)
]  # 6 emotions and 6 experts


# Change logging state
def change_logging_state(state):
    global is_logging
    is_logging = state
    ...
    for i in range(n_layers):
        ...
        with open(f, "r") as file:
            ...
            sns.heatmap(
                ...
                cmap="YlGnBu",
                xticklabels=[
                    f"Expert {i + 1}" for i in range(percentage_array.shape[1])
                ],
            ...
            plt.title(
                "Contribution of Each Expert for Each Emotion Layer " + str(i + 1)
            )
            plt.xlabel("Experts")
            plt.ylabel("Emotions")
