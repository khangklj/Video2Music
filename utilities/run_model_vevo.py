...
def train_epoch(cur_epoch, model, dataloader, 
    ...
    for batch_num, batch in enumerate(dataloader):
        ...
        x   = torch.tensor(batch["x"]).to(get_device())
        tgt = torch.tensor(batch["tgt"]).to(get_device())
        x_root   = torch.tensor(batch["x_root"]).to(get_device())
        tgt_root = torch.tensor(batch["tgt_root"]).to(get_device())
        x_attr   = torch.tensor(batch["x_attr"]).to(get_device())
        tgt_attr = torch.tensor(batch["tgt_attr"]).to(get_device())
        tgt_emotion = torch.tensor(batch["tgt_emotion"]).to(get_device())
        tgt_emotion_prob = torch.tensor(batch["tgt_emotion_prob"]).to(get_device())
        feature_semantic_list = torch.tensor(batch["semanticList"]).to(get_device())
